...
from tinyllm.common.io_struct import ReqRunStatus, FinishStatus
from tinyllm.utils.log_utils import init_logger
from tinyllm.common.io_struct import convert_sub_id_to_group_id
...
class InferBatch:
    ...
    def filter(self, request_ids: List[str], finished_request_ids: List[str]):
        ...
        if len(request_ids) == 0:
            self.free_self()
            return InferBatch(
                batch_id=self.batch_id, request_ids=[], req_manager=self.req_manage
            )
        free_req_index = []
        free_token_index = []
        ...
        return InferBatch(
            batch_id=self.batch_id, request_ids=request_ids, req_manager=self.req_manager
        )
